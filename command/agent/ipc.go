...
const (
	...
	authCommand            = "auth"
	statsCommand           = "stats"
	listMicroAppsCommand   = "list-microapps"
	listRoutersCommand     = "list-routers"
	updateRoutersCommand   = "update-routers"
...
func (i *AgentIPC) SetDiscoverd(discoverd *discoverd.Discoverd) {
	i.discoverd = discoverd
}

func (i *AgentIPC) Discoverd() *discoverd.Discoverd {
	return i.discoverd
}
...
func (i *AgentIPC) handleRequest(client *IPCClient, reqHeader *requestHeader) error {
	...
	case listMicroAppsCommand:
		return i.handleListMicroApps(client, seq)

	case listRoutersCommand:
		return i.handleListRouters(client, seq)

	case updateRoutersCommand:
		return i.handleUpdateRouters(client, seq)
...
func (i *AgentIPC) handleListMicroApps(client *IPCClient, seq uint64) error {
	header := responseHeader{
		Seq:   seq,
		Error: "",
	}
	resp := i.discoverd.ListMicroApps()
	return client.Send(&header, resp)
}

func (i *AgentIPC) handleListRouters(client *IPCClient, seq uint64) error {
	header := responseHeader{
		Seq:   seq,
		Error: "",
	}
	resp := i.discoverd.ListRouters()
	return client.Send(&header, resp)
}

func (i *AgentIPC) handleUpdateRouters(client *IPCClient, seq uint64) error {
	var rs []api.Router
	if err := client.dec.Decode(&rs); err != nil {
		return fmt.Errorf("decode failed: %v", err)
	}

	i.discoverd.UpdateRouters(rs)
	resp := responseHeader{
		Seq: seq,
	}
	return client.Send(&resp, nil)
