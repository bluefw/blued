...
func (c *RPCClient) ListMicroApps() ([]api.MicroApp, error) {
	header := requestHeader{
		Command: listMicroAppsCommand,
		Seq:     c.getSeq(),
	}
	var resp []api.MicroApp

	err := c.genericRPC(&header, nil, &resp)
	return resp, err
}

func (c *RPCClient) ListRouters() ([]api.Router, error) {
	header := requestHeader{
		Command: listRoutersCommand,
		Seq:     c.getSeq(),
	}
	var resp []api.Router
...
func (c *RPCClient) UpdateRouters(rs []api.Router) error {
	header := requestHeader{
		Command: updateRoutersCommand,
		Seq:     c.getSeq(),
	}

	return c.genericRPC(&header, rs, nil)
}
